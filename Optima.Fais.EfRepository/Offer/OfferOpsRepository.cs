...
{
    public class OfferOpsRepository : Repository<Model.OfferOp>, IOfferOpsRepository
    {
        public OfferOpsRepository(ApplicationDbContext context)
            : base(context, null)
        {
        }

        public override void Create(OfferOp item, string createdBy = null)
        {
            base.Create(item, createdBy);
        }

        public IEnumerable<Model.OfferOp> GetFiltered(AssetFilter assetFilter, string includes, int? assetId, string documentTypeCode, string assetOpState, DateTime startDate, DateTime endDate, string sortColumn, string sortDirection, int? page, int? pageSize, out int count)
        {

            IQueryable<Model.OfferOp> query = null;
            query = _context.OfferOps.AsNoTracking();
            ...
            if (assetId.HasValue) query = query.Where(a => a.OfferId == assetId);
            ...
            if (assetFilter.Filter != "" && assetFilter.Filter != null) query = query.Where(a => (a.Offer.Code.Contains(assetFilter.Filter) || a.Offer.Name.Contains(assetFilter.Filter)));
            ...
            if ((sortColumn != null) && (sortColumn.Length > 0) && (sortDirection != null) && (sortDirection.Length > 0))
            {
                query = sortDirection.ToLower() == "asc"
                    ? query.OrderBy(ExpressionHelper.GenericEvaluateOrderBy<Model.OfferOp>(sortColumn))
                    : query.OrderByDescending(ExpressionHelper.GenericEvaluateOrderBy<Model.OfferOp>(sortColumn));
            }
