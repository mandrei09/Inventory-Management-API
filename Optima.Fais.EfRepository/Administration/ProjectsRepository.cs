...
namespace Optima.Fais.EfRepository
{
	public class ProjectsRepository : Repository<Project>, IProjectsRepository
	{
		...
		private Expression<Func<Model.Project, bool>> GetFiltersPredicate(string filter)
		{
			Expression<Func<Model.Project, bool>> predicate = null;
			if ((filter != null) && (filter.Length > 0)) predicate = ExpressionHelper.GetAllPartsFilter(filter, _filterPredicate);

			return predicate;
		}

		public IEnumerable<Model.Project> GetByFilters(string filter, string includes, string sortColumn, string sortDirection, int? page, int? pageSize)
		{
