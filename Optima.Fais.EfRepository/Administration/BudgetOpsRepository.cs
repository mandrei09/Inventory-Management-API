...
{
    public class BudgetOpsRepository : Repository<Model.BudgetOp>, IBudgetOpsRepository
    {
        public BudgetOpsRepository(ApplicationDbContext context)
            : base(context, null)
        {
        }

        public override void Create(BudgetOp item, string createdBy = null)
        {
            base.Create(item, createdBy);
        }

        public IEnumerable<Model.BudgetOp> GetFiltered(AssetFilter assetFilter, string includes, int? assetId, string documentTypeCode, string assetOpState, DateTime startDate, DateTime endDate, string sortColumn, string sortDirection, int? page, int? pageSize, out int count)
        {

            IQueryable<Model.BudgetOp> query = null;
            query = _context.BudgetOps.AsNoTracking();
            ...
            if (assetFilter.Filter != "" && assetFilter.Filter != null) query = query.Where(a => (a.Budget.Code.Contains(assetFilter.Filter) || a.Budget.Name.Contains(assetFilter.Filter)));
            ...
            if ((sortColumn != null) && (sortColumn.Length > 0) && (sortDirection != null) && (sortDirection.Length > 0))
            {
                query = sortDirection.ToLower() == "asc"
                    ? query.OrderBy(ExpressionHelper.GenericEvaluateOrderBy<Model.BudgetOp>(sortColumn))
                    : query.OrderByDescending(ExpressionHelper.GenericEvaluateOrderBy<Model.BudgetOp>(sortColumn));
            }
