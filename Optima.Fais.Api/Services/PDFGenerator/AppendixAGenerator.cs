...
{
    public class AppendixAGenerator : IAppendixAGenerator
    {
        ...
        public AppendixAGenerator(IConfiguration configuration, IInventoryRepository inventoryRepository,
        ...
        public async Task<PdfDocumentResult> GenerateDocumentAsync(int inventoryId, ReportFilter reportFilter, DateTime? inventoryDateEnd)
        {
            ...
            var items = await this._assetRepository.GetInventoryListAsync(inventoryId, reportFilter);
        ...
        private void AddHeader(Section section, Model.Administration administration, 
            Model.Department department, Model.Division division, Model.CostCenter costCenter, 
            ReportFilter reportFilter, DateTime? inventoryDateEnd, decimal totalValue, bool complete = true)
        {
            ...
            if(reportFilter.AdministrationIds.Count > 0)
            {
				if (administration != null) paragraphInfo = administrationDetails;
            }
			else
			{
				if (reportFilter.CostCenterIds.Count > 0)
				{
            ...
            foreach (var item in items)
        {
			...
			if ((reportFilter.CostCenterIds != null) && (reportFilter.CostCenterIds.Count > 0))
			{
				...
				sumCurrentAPC = items.Sum(a => (reportFilter.CostCenterIds.Contains(a.CostCenterIdFinal)) ? a.CurrentAPC : 0);
			}
			else
			{
				if ((reportFilter.AdministrationIds != null) && (reportFilter.AdministrationIds.Count > 0))
				{
                    ...
                    sumCurrentAPC = items.Sum(a => (reportFilter.AdministrationIds.Contains(a.AdministrationIdFinal)) ? a.CurrentAPC : 0);
                }
			}
			...
			row.Cells[10].AddParagraph(String.Format(System.Globalization.CultureInfo.GetCultureInfo(this._cultureInfo), this._valueFormat, sumCurrentAPC));
