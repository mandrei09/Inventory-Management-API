...
{
    public class AppendixAMinusGenerator : IAppendixAMinusGenerator
    {
        ...
        public AppendixAMinusGenerator(IConfiguration configuration, IInventoryRepository inventoryRepository,
        ...
        public async Task<PdfDocumentResult> GenerateDocumentAsync(int inventoryId, ReportFilter reportFilter, DateTime? inventoryDateEnd)
        {
            ...
            var items = await this._assetRepository.GetInventoryMinusListAsync(inventoryId, reportFilter);
            ...
            //signatureEmployees = employeeCostCenters.Select(i => new SigningEmployeeDetail() { Employee = i.Employee, Info = "Gestionar" }).ToList();
            //var participantAreaList = this._documentHelperService.AddSignatureArea(signatureSection, "SIGNATURE_AREA", signatureEmployees, true);
            ...
            Model.Division division, Model.CostCenter costCenter, ReportFilter reportFilter, DateTime? inventoryDateEnd, decimal totalValue, bool complete = true)
        {
            ...
            //{
            //    string administrationDetails = (administration != null ? administration.Code : string.Empty);
            //    row2.Cells[11].AddParagraph($"Gestiune: { administrationDetails } \r\n Total NBV: {String.Format(System.Globalization.CultureInfo.GetCultureInfo(this._cultureInfo), this._valueFormat, totalAdministration)}");
            ...
            //else
            //{
            //    string administrationDetails = (administration != null ? administration.Code + " | " + administration.Name : string.Empty);
            //    row2.Cells[11].AddParagraph($"Gestiune: \r\n{ administrationDetails }");
            //    row2.Cells[11].MergeRight = 4;
            //    row2.Cells[11].Format.Alignment = ParagraphAlignment.Center;
            //    row2.Cells[11].VerticalAlignment = VerticalAlignment.Center;
            //    row2.Cells[11].Format.Font.Size = 8;
            ...
            paragraphInfo = "";
            //if (administration != null) paragraphInfo += $"\r\n Total NBV: {String.Format(System.Globalization.CultureInfo.GetCultureInfo(this._cultureInfo), this._valueFormat, totalValue)}";
            ...
            row5.Cells[2].AddParagraph("Unitate\r\nEconomica");
            ...
            row6.Height = "0.6cm";
            row6.Cells[0].Shading.Color = new Color(4, 50, 125);
            ...
            row6.Cells[1].Shading.Color = new Color(4, 50, 125);
        ...
        private void AddDocumentDetail(Section section, List<InventoryListAppendixA> items, ReportFilter reportFilter)
        {
            ...
            foreach (var item in items)
            {
                ...
                row.Cells[2].AddParagraph(item.Company == null ? string.Empty : item.Company); // Unitate Economica
                //row.Cells[2].AddParagraph((committeeType.Id == 2 || committeeType.Id == 10) ? item.InventoryNumber : item.IdleGroup); // Unitate Economica
                row.Cells[3].AddParagraph(item.InvNo == null ? string.Empty : item.InvNo); // Nr. SAP
				...
				row.Cells[10].AddParagraph(String.Format(System.Globalization.CultureInfo.GetCultureInfo(this._cultureInfo), this._valueFormat, item.CurrentAPC)); // Minus
				//row.Cells[10].AddParagraph(String.Format("{0:#,##0.##}", item.ValueInv)); // Pret Unitar
				...
				row.Cells[14].AddParagraph(String.Format(System.Globalization.CultureInfo.GetCultureInfo(this._cultureInfo), this._valueFormat, item.CurrBkValue)); // Minus
        ...
        //     private List<ParticipantDetail> AddSignatureArea(Section section, IEnumerable<CommitteeMember> members)
        //     {
        ...
        //         double currentTop = 0;
        //         double areaWidth = 30;
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        ...
        //         table.AddColumn(Unit.FromMillimeter(areaWidth));
        //         //table.AddColumn(Unit.FromMillimeter(areaWidth))
        //         var headerRow = AddSignatureTableHeader(table);


        //         var president = members.First(p => p.CommitteePosition.Code == this._committeePositionPresident);
        //         var secretary = members.First(p => p.CommitteePosition.Code == this._committeePositionSecretary);
        //         var internalMembers = members.Where(p => p.CommitteePosition.Code == this._committeePositionInternalMember).OrderBy(p => p.Employee.Email);
        //         var externalMembers = members.Where(p => p.CommitteePosition.Code == this._committeePositionExternalMember).OrderBy(p => p.Employee.Email);

        //         currentTop += headerRow.Height.Millimeter;
        //         var row = AddSignatureItem(table, $"{president.Employee.FirstName} {president.Employee.LastName}", $"{secretary.Employee.FirstName} {secretary.Employee.LastName}", members, externalMembers);
        //         participantAreaList.Add(new ParticipantDetail { Email = president.Employee.Email, SigningArea = new SigningArea { Top = currentTop, Left = 0, Height = row.Height.Millimeter, Width = areaWidth } });
        //         noOfRows += 1;



        //         //currentTop += row.Height.Millimeter;
        //         //row = AddSignatureItem(table, $"{secretary.Employee.FirstName} {secretary.Employee.LastName}", "");
        //         //participantAreaList.Add(new ParticipantDetail { Email = secretary.Employee.Email, SigningArea = new SigningArea { Top = currentTop, Left = 0, Height = row.Height.Millimeter, Width = areaWidth } });
        //         //noOfRows += 1;

        //         //foreach (var member in internalMembers)
        //         //{
        //         //    currentTop += row.Height.Millimeter;
        //         //    row = AddSignatureItem(table, $"{member.Employee.FirstName} {member.Employee.LastName}", "");
        //         //    participantAreaList.Add(new ParticipantDetail { Email = member.Employee.Email, SigningArea = new SigningArea { Top = currentTop, Left = 0, Height = row.Height.Millimeter, Width = areaWidth } });
        //         //    noOfRows += 1;
        //         //}
        ...
        //         headerRow.KeepWith = noOfRows;

        //         var administrationHeaderRow = AddSignatureAdministrationTableHeader(table);
        //         // administrationHeaderRow.KeepWith = noOfRows;
        //         return participantAreaList;
        //     }

        //     private Row AddSignatureTableHeader(Table table)
        //     {
        ...
        //row.VerticalAlignment = VerticalAlignment.Center;
        //row.Cells[0].AddParagraph("Comisia de inventariere");
        //row.Cells[0].Format.Alignment = ParagraphAlignment.Center;
        //row.Cells[0].Format.Font.Size = 8;
        //row.Cells[0].Format.Font.Bold = true;
        //         row.Cells[0].MergeRight = 7;
        //         //row.Cells[1].AddParagraph("Semnături:");
        //         //row.Cells[1].Format.Alignment = ParagraphAlignment.Center;
        //         //row.Cells[1].Format.Font.Size = 8;
        //         //row.Cells[1].Format.Font.Bold = true;

        //         //row.Cells[2].AddParagraph("Gestionar");
        ...
        //     private Row AddSignatureAdministrationTableHeader(Table table)
        //     {
        //         var row = table.AddRow();
        //         //row.HeightRule = RowHeightRule.Exactly;
        //         //row.Height = "0.5cm";
        //         row.Borders.Visible = true;
        //         row.VerticalAlignment = VerticalAlignment.Center;
        //         row.Cells[0].AddParagraph("Gestionari");
        //         row.Cells[0].Format.Alignment = ParagraphAlignment.Center;
        //         row.Cells[0].Format.Font.Size = 8;
        //         row.Cells[0].Format.Font.Bold = true;
        //         row.Cells[0].MergeRight = 7;
        ...
        //     private Row AddSignatureItem(Table table, string firstInfo, string secondInfo, IEnumerable<CommitteeMember> internalMembers, IEnumerable<CommitteeMember> externalMembers)
        //     {
        //         var row = table.AddRow();
        //         row.HeightRule = RowHeightRule.Exactly;
        //         row.Height = "2cm";
        ...
        //         row.Cells[0].AddParagraph(firstInfo);
        //         row.Cells[0].Format.Alignment = ParagraphAlignment.Center;
        //         row.Cells[0].Format.Font.Size = 8;
        //         row.Cells[0].Format.Font.Bold = true;
        ...
        //row.Cells[1].AddParagraph(secondInfo);
        //row.Cells[1].Format.Alignment = ParagraphAlignment.Center;
        //row.Cells[1].Format.Font.Size = 8;
        //row.Cells[1].Format.Font.Bold = true;
        ...
        //foreach (var member in internalMembers)
        //{
        //             row.Cells[index].AddParagraph(member.Employee.FirstName + " " + member.Employee.LastName);
        //             row.Cells[index].Format.Alignment = ParagraphAlignment.Center;
        //             row.Cells[index].Format.Font.Size = 8;
